...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      const others = [
        { name: 'price_tables', setter: setPriceTables },
        { name: 'instrument_types', setter: setInstrumentTypes },
        { name: 'certificate_masks', setter: setCertificateMasks },
        { name: 'procedures', setter: setProcedures },
        { name: 'payment_methods', setter: setPaymentMethods },
        { name: 'banks', setter: setBanks },
        { name: 'units_of_measure', setter: setUnitsOfMeasure },
        { name: 'document_templates', setter: setDocumentTemplates },
      ];
      
      let templates: DocumentTemplate[] = [];
      for (const { name, setter } of others) {
        const res = await fetch(`/api/mock/${name}`);
        if (res.ok) {
          const data = await res.json();
          setter(data);
          if (name === 'document_templates') templates = data;
        }
      }
      ...
      for (const req of requiredTypes) {
        if (!templates.find(t => t.id === req.id || t.applyTo === req.applyTo)) {
          ...
          await fetch('/api/mock/document_templates', {
            method: 'POST',
      ...
      if (templates.length < requiredTypes.length) {
        const res = await fetch('/api/mock/document_templates');
        if (res.ok) setDocumentTemplates(await res.json());
      }
    } catch (e) {
