...
import { XCircle, Plus, FileText, Save } from 'lucide-react';
...
const DocumentTemplateEditor: React.FC<DocumentTemplateEditorProps> = ({ isOpen, onClose, template, onSave }) => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, field: 'letterheadBase64' | 'footerBase64') => {
      ...
      <div className="bg-white dark:bg-slate-800 w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-[3rem] border border-slate-100 dark:border-slate-700 shadow-2xl p-8 space-y-8 animate-in zoom-in-95 duration-300">
        ...
        <div className="grid grid-cols-1 gap-8">
          ...
          <div className="space-y-3">
            ...
            <div className="relative flex items-center justify-center p-8 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-[2.5rem] bg-slate-50 dark:bg-slate-900/50 hover:bg-slate-100 dark:hover:bg-slate-900 transition-all">
              <input type="file" accept="image/*" onChange={e => handleImageUpload(e, 'letterheadBase64')} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
              {form.letterheadBase64 ? (
                ...
                <div className="flex flex-col items-center text-slate-400">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest">Subir Layout</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-700">
          <button 
            onClick={onClose}
            className="px-8 py-4 text-slate-500 font-black text-[10px] uppercase tracking-widest hover:bg-slate-100 dark:hover:bg-slate-700 rounded-2xl transition-all"
          >
            Cancelar
          </button>
          <button 
            onClick={handleSave}
            className="flex items-center gap-2 px-10 py-4 bg-purple-600 hover:bg-purple-700 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-purple-500/20 transition-all hover:scale-105 active:scale-95"
          >
